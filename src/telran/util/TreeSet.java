...
public class TreeSet<T> implements SortedSet<T> {
    ...
    public void displayAsDirectory() {
        displayAsDirectory("", root, false);

    }

    private void displayAsDirectory(String prefix, Node<T> junction, boolean isLeft) {
        if (junction != null) {
            System.out.println (prefix + (isLeft ? "|-- " : "\\-- ") + junction.obj);
            displayAsDirectory(prefix + (isLeft ? "|   " : "    "), junction.left, true);
            displayAsDirectory(prefix + (isLeft ? "|   " : "    "), junction.right, false);
        }
    }
    ...
    private void inversion(Node<T> junction) {
        if (junction == null) {
            return;
        }
        swap(junction);
        inversion(junction.left);
        inversion(junction.right);
    }
