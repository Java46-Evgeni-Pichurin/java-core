...
public class TreeSet<T> extends AbstractCollection<T> implements SortedSet<T> {
    ...
    public T ceiling(T pattern) {
        return getCelling(root, pattern);
    }

    @Override
    public T floor(T pattern) {
        return getFloor(root, pattern);
    }


    private T getFloor(Node<T> root, T pattern) {
        if (root.right == null && root.left == null && root.obj != pattern) {
            if (comp.compare(root.obj, pattern) < 0) {
                return root.obj;
            }
            return null;
        }
        ...
        else if (comp.compare(root.obj, pattern) < 0) {
            return root.right == null ? root.obj : getFloor(root.right, pattern);
        } else {
            return root.left == null ? root.obj : getFloor(root.left, pattern);
        }
    }

    private T getCelling(Node<T> root, T pattern) {
        if (root.right == null && root.left == null && root.obj != pattern) {
            if (comp.compare(root.obj, pattern) > 0) {
                return root.obj;
            }
        ...
        if (root.obj == pattern) {
            ...
            return root.left == null ? root.obj : getCelling(root.left, pattern);
        } else {
            return root.right == null ? root.obj : getCelling(root.right, pattern);
        }
    }
